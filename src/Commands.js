...
class Commands {
    ...
    register(command, callback) {
        ...
        for (let item of this.commands) {
            let itemAliases = this._checkCommandAliases(item.constructor.name, item.aliases);
            if (itemAliases.some((elem) => {
                return commandAliases.indexOf(elem) > -1;
            })) {
                let error = new Error("One of the aliases or command names are already registered.");
                callback(error);
